...
// DPFuzz global variables = start =
FILE *predictionFILE;
char DPFuzztmpPath[500];
float funcPrediction[NEW_MAP_SIZE + 1];
int totalInsFunc = 0;
// DPFuzz global variables =  end  =
...
EXP_ST void setup_shm(void)
{
  ...
  shm_id = shmget(IPC_PRIVATE, MAP_SIZE + NEW_MAP_SIZE, IPC_CREAT | IPC_EXCL | 0600);
...
static u8 run_target(char **argv, u32 timeout)
{
  ...
  memset(trace_bits, 0, MAP_SIZE + NEW_MAP_SIZE);
  MEM_BARRIER();
...
#ifndef SIMPLE_FILES
    fn = alloc_printf("%s/hangs/id:%06llu,%s,%llu", out_dir,
...
static u32 calculate_score(struct queue_entry *q)
{
  ...
  // DPFuzz calculate score = start =
  int coverCount = 0;
  float testCasePrediction = 0;
  for (int i = 0; i < totalInsFunc; i++)
  {
    int funcCover = *(u8 *)(trace_bits + MAP_SIZE + i);
    if (funcCover != 0)
    {
      coverCount++;
      testCasePrediction = testCasePrediction + funcPrediction[i];
    }
  }
  if (testCasePrediction != 0.0)
  {
    perf_score = perf_score + perf_score * (testCasePrediction / coverCount);
    OKF("%d", perf_score);
  }
  // DPFuzz calculate score =  end  =
  ...
  for (stage_cur = 0; stage_cur < stage_max; stage_cur++)
  {
    ...
    for (i = 0; i < use_stacking; i++)
    {
      ...
      case 0:
        ...
        if ((posn = get_random_modifiable_posn(1, 1, temp_len, branch_mask, position_map)) == 0xffffffff)
          break;
      ...
      case 1:
        ...
        if ((posn = get_random_modifiable_posn(8, 1, temp_len, branch_mask, position_map)) == 0xffffffff)
          break;
      ...
      case 2:
        ...
        if ((posn = get_random_modifiable_posn(16, 1, temp_len, branch_mask, position_map)) == 0xffffffff)
          break;
        if (UR(2))
        {
      ...
      case 3:
        ...
        if ((posn = get_random_modifiable_posn(32, 1, temp_len, branch_mask, position_map)) == 0xffffffff)
          break;
        if (UR(2))
        {
...
int main(int argc, char **argv)
{
  ...
  while ((opt = getopt(argc, argv, "+bq:rsz:i:o:f:m:t:T:dnCB:S:M:x:Q")) > 0)

    switch (opt)
    {
    case 'z':
      strcpy(DPFuzztmpPath, optarg);
      SAYF(DPFuzztmpPath);
      break;
  ...
  check_asan_opts();
  // ===============================================================================
  // DPFuzz get initial data = start =
  // ===============================================================================
  struct stat st = {0};
  if (stat(DPFuzztmpPath, &st) == -1)
    FATAL("Middleware folder does not exist! Please do instrumentation first ...");
  char ins_filePath[500];
  strcpy(ins_filePath, DPFuzztmpPath);
  strcat(ins_filePath, "/instrumentation.txt");
  FILE *insfp = fopen(ins_filePath, "r");
  if (insfp == NULL)
    FATAL("minstrumentation.txt does not exist! Please do instrumentation first ...");
  char fileLineStr[NEW_MAP_SIZE] = {0};
  while (!feof(insfp))
  {
    fgets(fileLineStr, sizeof(fileLineStr) - 1, insfp);
    totalInsFunc++;
  }
  if (totalInsFunc == -1)
    FATAL("The number of functions for instrumenting is 0! Please do instrumentation first ...");
  totalInsFunc--;
  fclose(insfp);
  char fpPath[500];
  strcpy(fpPath, DPFuzztmpPath);
  strcat(fpPath, "/prediction.txt");
  predictionFILE = fopen(fpPath, "r");
  char predictionStr[100];
  int flag = 0;
  while (fgets(predictionStr, 100, predictionFILE) != NULL)
  {
    funcPrediction[flag] = atof(predictionStr);
    flag++;
  }
  fclose(predictionFILE);
  // ===============================================================================
  // DPFuzz get initial data =  end  =
  // ===============================================================================
